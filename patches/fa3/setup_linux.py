...
#
# Refactored replacement for upstream hopper/setup.py, copied over the clone
# by build_linux.sh (full file copy, not a patch). Windows builds use
...
def download_and_copy(name, src_func, dst_path, version, url_func):
    ...
    url = url_func("linux", arch, version)
    src_path = src_func("linux", arch, version)
